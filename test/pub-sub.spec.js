describe('Pub Sub Tests', function () {
  ...
  it('should have 2 registered subscribers', function () {
    var ps = new PubSub();
    ps.subscribe('main', 'item.changed', function () {});
    ps.subscribe('main', 'item.changed', function () {});

    expect(ps.count()).toBe(2);
  });

  it('should have 3 total registered subscribers subscribe two channels', function () {
    var ps = new PubSub();
    ps.subscribe('main', 'item.changed', function () {});
    ps.subscribe('main', 'item.changed', function () {});
    ps.subscribe('private', 'item.changed', function () {});
  ...
  it('should throw an error because a valid callback function was not provided', function () {
    ...
    expect(function () {
      ps.subscribe('main', 'item.changed', 'callback');
    }).toThrow(new Error('you must provide a subscriber callback function'));
  });

  it('should unregister a subscription', function () {
    var ps = new PubSub();
    var s1 = ps.subscribe('main', 'item.added', function () {});
    var s2 = ps.subscribe('main', 'item.changed', function () {});

    ps.unsubscribe('main', 'item.changed', s2);
  ...
  it('should output the current channel list and subscriber count', function() {
    var ps = new PubSub();
    var s1 = ps.subscribe('main', 'item.added', function () {});
    var s2 = ps.subscribe('main', 'item.added', function () {});
    var s3 = ps.subscribe('other', 'item.added', function () {});
  ...
  it('should register two callbacks and execute one callback', function () {
    ...
    var ps = new PubSub();
    ps.subscribe('main', 'item.added', function () {
      data.called1 = true
    });
    ps.subscribe('main', 'item.changed', function () {
      data.called2 = true
  ...
  it('should receive a callback with data', function () {
    ...
    var ps = new PubSub();
    ps.subscribe('main', 'item.added', function (response) {
      data = response;
    });
    ps.notify('main', 'item.added', {name: 'foo', hasData: true});
